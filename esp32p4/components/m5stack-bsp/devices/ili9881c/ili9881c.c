...
static void **get_framebuffers(bsp_display_t *self) {
    return ((ili9881c_lcd_t *)self)->frame_buffers;
}
...
esp_err_t ili9881c_lcd_create(const bsp_display_config_t *config, bsp_display_t **out) {
    esp_err_t ret;

    ili9881c_lcd_t *state = calloc(1, sizeof(ili9881c_lcd_t));
    if (state == NULL) {
