...
esp_err_t ili9881c_lcd_init(const ili9881c_lcd_config_t *config, ili9881c_lcd_t *lcd) {
    esp_err_t ret;

    struct ili9881c_lcd_state *state = calloc(1, sizeof(struct ili9881c_lcd_state));
    if (state == NULL) {
    ...
    ret = esp_lcd_new_panel_ili9881c(state->io, &panel_config, &state->panel);
...
esp_err_t ili9881c_lcd_deinit(ili9881c_lcd_t lcd) {
...
esp_err_t ili9881c_lcd_set_brightness(ili9881c_lcd_t lcd, int brightness) {
...
esp_err_t ili9881c_lcd_draw_bitmap(ili9881c_lcd_t lcd, bsp_rect_t rect, const void *data) {
...
esp_err_t ili9881c_lcd_flush(ili9881c_lcd_t lcd, int fb_index) {
...
void **ili9881c_lcd_get_frame_buffers(ili9881c_lcd_t lcd) {
