...
保つ。Tab5 側は受信スレッドでは PCM をリング、Opus をパケット境界つきキューへコピーするだけにする。
Opus decode taskは別coreで20 ms PCM frameキューへ変換し、音声出力taskは独立して
`bsp_audio_write`（I2S DMAで自然ペーシング）へ吐き出す。5 decoded frame = 100 msを貯めてから開始・
アンダーラン復帰し、入力・PCMの各queueは最大50 frame = 1秒を保持する。上限超過時は古いデータを
100 ms分までまとめて落とし、遅延の固定化を避ける。
