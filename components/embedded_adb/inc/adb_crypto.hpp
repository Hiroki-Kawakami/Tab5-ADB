...
public:
    ...
    // Self-signed X.509 certificate (DER) wrapping this key, for ADB STARTTLS and
    // six-digit pairing. adbd matches the client cert by its public-key
    // fingerprint, so the subject and validity window are fixed (the device has
    // no RTC to date a cert).
